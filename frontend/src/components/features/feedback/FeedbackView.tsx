...
export function FeedbackView({ brand }: FeedbackViewProps) {
  ...
  const handleRating = (rate: number) => {
    ...
    if (rate >= 4) {
      if (!brand.payment_link) { // Using payment_link as a proxy if review_link is missing in new schema
        setError('Review link not available');
        return;
      }
      if (!isValidHttpUrl(brand.payment_link)) {
        setError('Invalid review URL provided');
        return;
      }

      try {
        if (!openPopup(brand.payment_link)) {
          setError(
      ...
      if (!validateWhatsapp(brand.whatsapp_number)) {
