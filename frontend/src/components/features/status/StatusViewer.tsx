...
export function StatusViewer({ status }: StatusViewerProps) {
  ...
  const renderContent = () => {
    switch (currentStatus.type) {
      case 'image':
        return <img src={currentStatus.content} alt="Status" className="w-full h-full object-contain" />;
      case 'video':
        return <video src={currentStatus.content} autoPlay muted playsInline className="w-full h-full object-contain" />;
      case 'text':
        return (
          <div className="flex items-center justify-center h-full p-8 bg-gradient-to-br from-gray-700 via-gray-900 to-black">
